...
# ===== NOTES =====
# It appears that we cannot set the framerate of the arducam using the OpenCV v4l2 plugin.
# Please use `arducam-test-gst.py` instead.
# =================
...
def main():
    ...
    #     cap = cv.VideoCapture("v4l2src device=/dev/v4l/by-id/usb-Arducam_Technology_Co.__Ltd._Arducam_OV9782_USB_Camera_UC852-video-index0 ! videorate ! videoconvert! videoscale ! video/x-raw,framerate=30/1 ! appsink")

    cap = cv.VideoCapture("/dev/v4l/by-id/usb-Arducam_Technology_Co.__Ltd._Arducam_OV9782_USB_Camera_UC852-video-index0")
